    ...
    <div className="space-y-10">
      ...
      <TopicForm
        onCreated={(updated) => {
          load();
          if (updated?.id != null) {
            setEditing((e) => (e?.id === updated.id ? { ...e, ...updated } : e));
          }
        }}
        editing={editing}
                ...
                {!loading &&
                  items.map((t) => (
                    <tr key={t.id} className="hover:bg-forge-800/25 transition-colors">
                      ...
                      <td className="px-4 py-4 text-slate-400 align-top tabular-nums">{t.target_count}</td>
                      <td className="px-4 py-4 align-top">
                        <span
                          className={
                            t.is_active
                      ...
                      <td className="px-4 py-4 text-right align-top space-x-1">
                        <button type="button" onClick={() => setEditing(t)} className="cf-btn-ghost text-xs text-sky-400 hover:text-sky-300">
                          Edit
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            api.topics.remove(t.id).then(load).catch((e) => alert(e.response?.data?.detail || e.message))
                          }
